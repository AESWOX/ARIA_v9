...
import { SelectionSwitcher } from "@vendor/ui/ui/components/selection-switcher";
import { Spinner } from "@vendor/ui/ui/components/spinner";
...
/**
 * Route pages are lazy-loaded (P1.2 — bundle splitting). Each page ships in
 * its own chunk, so the heavy deps they pull in (xterm via ChatPage, the
 * per-page data grids, dialogs, …) never touch the initial main bundle.
 * `buildRoutes` renders them inside a <Suspense> boundary further down.
 */
...
// ChatPage is rendered persistently (kept mounted across tab switches so the
// PTY child / WebSocket / xterm instance survive) — see the host block near
// the bottom of this file. It is STILL lazy: the xterm bundle only downloads
// once the host first mounts, keeping it out of the initial main chunk.
const ChatPage = lazy(() => import("@/pages/ChatPage"));
import { PageSkeleton, TerminalSkeleton } from "@/components/Skeleton";
 ...
 */
/** Route element factory: eager `ComponentType` or lazy-loaded page chunk. */
...
const BUILTIN_NAV_REST: NavItem[] = [
  ...
  { path: "/mcp", label: "MCP", icon: Plug, group: "settings" },
  { path: "/channels", label: "Channels", icon: Radio, group: "settings" },
  { path: "/webhooks", label: "Webhooks", icon: Webhook, group: "settings" },
  { path: "/pairing", label: "Pairing", icon: ShieldCheck, group: "settings" },
  {
    path: "/profiles",
    labelKey: "profiles",
    label: "Profiles",
  ...
  { path: "/system", label: "System", icon: Wrench, group: "settings" },
  {
    path: "/docs",
...
export default function App() {
  ...
  // P2.1: non-primary sidebar section starts collapsed unless a settings route is active.
  const [settingsOpen, setSettingsOpen] = useState(
    () => !["/sessions", "/files", "/analytics"].includes(pathname),
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden pt-14 lg:pt-0">
        <a
          href="#app-main"
          className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-[100] focus:rounded focus:border focus:border-current/30 focus:bg-background-base focus:px-3 focus:py-1.5 focus:text-sm focus:text-midground"
        >
          {t.app.skipToContent ?? "Skip to content"}
        </a>
        <div id="app-main" className="flex min-h-0 min-w-0 flex-1">
          <aside
            id="app-sidebar"
          ...
          >
            ...
            >
              ...
              {sidebarNav.coreItems.some((item) => item.group === "settings") && (
                <div
                  aria-labelledby="aria-sidebar-settings-heading"
                ...
                >
                  <button
                    ...
                    className={cn(
                      "group/settings flex items-center justify-between gap-2",
                      "px-5 pt-2.5 pb-1",
                    ...
                    id="aria-sidebar-settings-heading"
                    onClick={() => setSettingsOpen((v) => !v)}
                    type="button"
                  >
                    <span>{t.app.settingsNavSection ?? "Settings"}</span>
                    <span
                      aria-hidden
                      className={cn(
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="aria-sidebar-plugin-nav-heading"
                  className="flex flex-col border-t border-current/10 pb-2"
                  role="group"
                >
                  <span
                    ...
                    id="aria-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateARIA = status?.can_update_aria === true;
  ...
  if (canUpdateARIA) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateARIA,
      runningLabel: t.status.updatingARIA,
      spin: false,
    });
  }
...
interface NavItem {
  ...
  labelKey?: string;
  path: string;
  /** P2.1: non-primary items collapse under a "Settings" group in the sidebar. */
  group?: "settings";
}
