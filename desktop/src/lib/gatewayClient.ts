...
export class GatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    } else if (window.__ARIA_AUTH_REQUIRED__) {
      const { ticket } = await getWsTicket();
      authParamName = "ticket";
      authParamValue = ticket;
    } else {
      authParamName = "token";
      // Tauri mode: the WS endpoint (/ws) doesn't require auth; use the
      // runtime token when available, otherwise fall back to the injected
      // session token (browser mode).
      authParamValue = getRuntimeToken() ?? window.__ARIA_SESSION_TOKEN__ ?? "";
      if (!authParamValue) {
        this.setState("error");
        throw new Error(
          "Session token not available — page must be served by the ARIA dashboard",
        );
      }
    }
