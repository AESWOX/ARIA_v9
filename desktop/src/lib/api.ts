...
// (e.g. https://ARIA/). The Python backend
// injects ``window.__ARIA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
//
// Same-origin SPA mode: the backend serves desktop/dist itself, so the
// webview and the API share one origin. The runtime token is embedded into
// the served HTML as <meta name="runtime-token"> — no Tauri invoke bridge
// is involved in API authentication anymore.
let _runtimeBaseUrl: string | null = null;
let _runtimeToken: string | null = null;

function readRuntimeToken(): string | null {
  if (typeof window === "undefined") return null;
  const meta = document.querySelector('meta[name="runtime-token"]');
...
export async function initRuntimeBaseUrl(): Promise<string> {
  ...
  _runtimeBaseUrl = readBasePath();
  return _runtimeBaseUrl;
}

/** The runtime token the backend embedded into the served HTML. */
export function getRuntimeToken(): string | null {
  // Lazy-load: if initRuntimeBaseUrl() hasn't been awaited yet (e.g. a
  // WebSocket connect fired before the first REST call), read the token
  // straight from the DOM — it's embedded in the served page anyway.
...
function readBasePath(): string {
  ...
  // from the backend, so relative URLs are correct — no hardcoded port.
  const raw = window.__ARIA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ARIA_BASE_PATH = readBasePath();
...
declare global {
  interface Window {
    __ARIA_SESSION_TOKEN__?: string;
    __ARIA_BASE_PATH__?: string;
    /** Server-injected runtime token (same-origin SPA mode): embedded into
     * the served index.html as <meta name="runtime-token">. Legacy inline
     * fallback kept for browser/server-rendered deployments. */
    __RUNTIME_TOKEN__?: string;
    ...
    __ARIA_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-ARIA-Session-Token";
...
export async function fetchJSON<T>(
  ...
  // Resolve the runtime base first — this also caches the runtime token in
  // Tauri mode, so the header below is populated even on the first request.
  const base = await resolveBase();
  // Inject the session token into all /api/ requests.
  const headers = new Headers(init?.headers);
  // Tauri mode: the backend authenticates via X-Local-Agent-Token, which is
  // the runtime token Rust generated and passed to the sidecar. The legacy
  // X-ARIA-Session-Token header only exists when the server injected
  // __ARIA_SESSION_TOKEN__ into a server-rendered index.html (browser mode).
  if (_runtimeToken && !headers.has("X-Local-Agent-Token")) {
    headers.set("X-Local-Agent-Token", _runtimeToken);
  }
  const token = window.__ARIA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``aria update``, ``aria gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ARIA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ARIA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("aria.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("aria.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__ARIA_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("aria.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__ARIA_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the ARIA dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``aria.web_server._ws_auth_ok``)
...
export async function getWsTicket(): Promise<{ ticket: string; ttl_seconds: number }> {
  const res = await fetch(`${await resolveBase()}/api/auth/ws-ticket`, {
    method: "POST",
    credentials: "include",
  });
  if (!res.ok) {
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ARIA_AUTH_REQUIRED__) {
    const { ticket } = await getWsTicket();
    return ["ticket", ticket];
  }
  // Tauri mode: the WS endpoint (/ws) doesn't require auth, but pass the
  // runtime token along for forward-compat and parity with REST headers.
  const token = _runtimeToken ?? window.__ARIA_SESSION_TOKEN__ ?? "";
  return ["token", token];
}
 ...
 *  - loopback / ``--insecure``: attach the ``X-ARIA-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``aria_session_at`` cookie rides along via ``credentials: 'include'``.
 *
...
export async function authedFetch(
  ...
  return fetch(`${await resolveBase()}${url}`, {
    ...init,
    headers,
    credentials: init?.credentials ?? "include",
  });
 ...
 * ``window.__ARIA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  const base = await resolveBase();
  // Same-origin mode: base is "" — derive host/protocol from window.location.
  const proto = base
...
export const api = {
  ...
  logout: async () =>
    fetch(`${await resolveBase()}/auth/logout`, {
      method: "POST",
      credentials: "include",
    }).then((r) => {
  ...
  updateARIA: () =>
    fetchJSON<ActionResponse>("/api/aria/update", { method: "POST" }),
  checkARIAUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/aria/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** aria-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``aria update``. */
  can_update_aria?: boolean;
  config_path: string;
