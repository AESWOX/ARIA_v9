/**
 * ARIA Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__ARIA_PLUGIN_SDK__`` and
 * ``window.__ARIA_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__ARIA_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@vendor/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``ARIAPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@aria/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @vendor/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__ARIA_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__ARIA_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__ARIA_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ARIAPluginSDK {
...
declare global {
  interface Window {
    __ARIA_PLUGIN_SDK__?: ARIAPluginSDK;
    __ARIA_PLUGINS__?: PluginRegistry;
  }
}
