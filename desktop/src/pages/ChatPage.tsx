/**
 * ChatPage — embeds `aria` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (aria/web_server.py)                          .
 *          ▼                                                              .
...
async function buildWsUrl(
  ...
  // Profile-scoped chat: the PTY child gets ARIA_HOME pointed at the
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ARIA_SESSION_TOKEN__ &&
    !window.__ARIA_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `aria dashboard`, not directly."
      : null,
  );
  ...
  // P2.4: true once the PTY WebSocket reaches OPEN; drives the terminal
  // skeleton overlay so the first paint shows a shimmer, not an empty box.
  ...
  useEffect(() => {
    ...
    // Tauri mode: runtime token from Rust bootstrap; browser mode: injected.
    const token = getRuntimeToken() ?? window.__ARIA_SESSION_TOKEN__;
    const gated = !!window.__ARIA_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner ARIA TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[aria-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__ARIA_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      const url = await buildWsUrl(authParam, resumeParam, channel, scopedProfile);
    ...
    // forwarding them into ARIA. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="aria-chat-xterm-host min-h-0 min-w-0 flex-1"
          />

          {/* P2.4: shimmer overlay while the PTY WebSocket is still
              connecting — replaced by live terminal output on onopen. */}
          {!wsConnected && !sessionEnded && !banner && (
